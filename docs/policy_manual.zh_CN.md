...
|              |         |                      |禁用 mount 系统调用<br><br>`disallow-mount`|ALL|[MOUNT(2)](https://man7.org/linux/man-pages/man2/mount.2.html) 常被用于权限提升、容器逃逸等攻击，而几乎所有的微服务应用都无需 mount 操作。因此建议使用此规则限制容器内进程使用 mount 系统调用。|禁用 mount 系统调用|AppArmor<br>BPF
