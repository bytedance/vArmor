...
* **Network Proxy Enforcer**. vArmor introduces a sidecar-proxy-based enforcer (powered by Envoy) that transparently intercepts and controls container network egress traffic at L4 (TCP), L7 (HTTP), and TLS SNI levels. It supports both allow-list and deny-list modes, audit logging, and dynamic policy updates without Pod restarts.
* **AI Agent Protection**. vArmor provides defense-in-depth for AI Agent workloads by combining kernel-level mandatory access control (AppArmor/BPF/Seccomp) with application-protocol-level network access control (NetworkProxy), effectively mitigating risks such as prompt injection-induced tool abuse and unauthorized data exfiltration.
...
```bash
helm install varmor varmor-0.10.0.tgz \
