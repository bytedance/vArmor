...
为了进一步在真实场景中针对不同的 Enforcer 进行对比测试，我们使用 Phoronix 测试套件（Phoronix Test Suite，简称 PTS）
针对一些常见负载（Redis、Apache 等）进行了一系列自动化性能测试。
...
本轮测试，我们对 Apparmor、bpf lsm 这两种 enforcer LSM 进行横向对比测试，每个 enforcer 都选取三种典型场景，包括 AlwaysAllow、RuntimeDefault、EnhanceProtect最强防护,每个场景的 policy 如下：

- Init 测试
  - 不应用任何策略
- AlwaysAllow
  - 使用AlwaysAllow Mode 进行测试，不开启任何 rule
- RuntimeDefault
  - 使用RuntimeDefault Mode 进行测试，不开启任何 rule
- EnhanceProtect最强防护
  - 使用EnhanceProtect Mode 进行测试，开启如下 rule
    ...
    - runc-override-mitigation
      此外，我们也对 Seccomp enforcer 进行了测试，开启了目前提供的 4 条 rule。该测试仅供参考，不作为性能测试的基准。
      测试用到的 policy 文件可以在 test/perf/policy 目录下找到。
...
- 记录测试结果。
  特别地，对于 Init、 BPF 和 Seccomp 模式，我们使用了不同的 Pod 配置，开启了 ``container.apparmor.security.beta.kubernetes.io/phoronix: unconfined`` 来确保没有开启 Apparmor，避免默认开启的 Apparmor 影响测试结果。
  可以在  test/perf/policy 目录下找到 pod 定义和 Phoronix 运行配置。

  自动化测试脚本你可以在 test/perf/policy 目录下找到。此外我们针对 sysbench 和 unixbench 也编写了单独的测试脚本，如果你感兴趣也可以自行进行测试。
#### 测试结果

1. **EnhanceProtect**:
   
   - BPF 的性能相比 AppArmor 下降了约 1.2%。

2. **RuntimeDefault**:
   
   - BPF 的性能相比 AppArmor 下降了约 0.6%。

3. **AlwaysAllow**:
   
   - BPF 的性能相比 AppArmor 下降了约 0.1%。

分析表明，虽然 BPF 相对于 AppArmor 在不同场景下通常表现出轻微的性能下降，但差异相对较小。这表明 BPF 是 AppArmor 的可行替代方案，在安全应用中具有可接受的性能损耗。
