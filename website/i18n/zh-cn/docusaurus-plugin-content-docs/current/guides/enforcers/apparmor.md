...
AppArmor 支持在内核层限制 socket 操作；如果还需要按目标 IP 或端口控制连接，可选择 [BPF](bpf.md)。

## 使用前提 {#before-you-start}

节点必须启用 AppArmor LSM，最低内核要求为 4.15。检查[安装说明](../../getting_started/installation.md)以及所有可能承载工作负载的节点和运行时。启用 vArmor 组件不会自动启用缺失的内核 LSM。
