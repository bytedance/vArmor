...
- 按需使用 `allow`、`deny`、`audit`、`allow,audit` 或 `deny,audit`。不要在同一条规则中同时使用 allow/deny，也不能依赖每条处理路径都会拒绝全部非法限定符。

同一条规则中不同条件为 **AND**；同一条件列表中的多个值为 **OR**；allow 或 deny 组内的不同规则为 **OR**。不同规则的局部条件不会被拼接成一条新规则。

**在同时适用的链内，L4 allow 与 HTTP allow 是可独立成立的授权。** 对整个子网/端口的 L4 allow 可以放行不满足 HTTP allow 的请求。不能把宽泛的 L4 allow 当作狭窄 HTTP 允许列表的必需前置条件；匹配的 deny 仍优先。
...
| `httpRules[].match.ports` | 实际目标端口，并结合生成的 authority 条件 | 实际目标端口 |
...
`methods: [GET]` 不匹配 `get` 或 `GeT`。v0.10.5 保留合法自定义方法 token，不再把策略值转大写。此语义针对合法 HTTP，不保证畸形流量一定被识别为 HTTP。
...
端口匹配实际目标端口，不能只靠伪造 Host 端口满足条件。显式非默认端口还要求相应的 authority 端口形式。优先使用含实际目标端口的正常 URL；向其他端口连接时伪造 `Host: ...:443` 不能满足目标 443 规则。

## 选链与 MITM 范围 {#chain-selection-and-mitm-scope}

`mitm.domains` 选择解密范围，**不是允许列表**。被拦截的请求仍需通过 HTTP/L4 授权。

Envoy 先按目标 IP，再按 SNI/协议等条件筛选候选链。v0.10.5 在 MITM IP 分支内补齐明文 HTTP 和 DNS TLS 候选链，所以声明 IP MITM 不会让该 IP 的明文 HTTP 跳过 HTTP 规则。DNS/IP 重叠不能解释成全局 SNI 优先。

| 情况 | 处理方式 |
| --- | --- |
| 发往 MITM IP 的明文 HTTP | 仍执行 HTTP 规则 |
| 使用已配置 DNS SNI 的 TLS，包括目标也是 MITM IP | DNS MITM 候选链处理解密 HTTP |
| 发往 MITM IP 且无 SNI 的 TLS | IP MITM 候选链处理解密 HTTP |
| 不在适用 MITM 范围内的 TLS | TLS 透传；加密 HTTP 不可见 |
| 非 HTTP、非 TLS 的 TCP | TCP 规则 |

## 域前置缓解边界 {#domain-fronting-boundary}

MITM 没有 catch-all 虚拟主机。HTTP authority 不属于所选链的虚拟主机集合时，没有上游路由，通常返回 404；范围内仍执行 HTTP/L4 规则。

这**不要求下游 SNI 与 HTTP Host 一一相等**。A/B 都在 DNS MITM 链覆盖范围内时，SNI=A、Host=B 可以按 B 的规则继续处理。IP 虚拟主机集合也不严格绑定每个目标 IP 与同名 Host。

上游 MITM TLS 根据路由的 HTTP authority 设置 SNI 并校验证书身份，但 cluster 仍使用原始目标地址；不会重新按 Host 解析 IP，也不建立 DNS/IP 绑定。TLS 透传无法执行这些解密后的 Host 检查。
