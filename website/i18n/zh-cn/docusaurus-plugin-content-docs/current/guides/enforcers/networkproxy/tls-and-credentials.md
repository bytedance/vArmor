...
环境变量不代表所有 SDK 自动兼容。已有信任变量、自定义 trust store、证书固定和 CA 缓存可能需要应用配置。挂载的证书文件更新后，业务进程不一定自动重载信任库；验证 MITM 不应使用 `curl -k`。

通常从公共可信上游开始。私有上游 CA 是另一项信任需求，不能与应用到代理的信任混淆。当前策略 API 没有通用上游 CA 引用字段。请通过策略 API 修改支持的配置；直接编辑生成的配置可能被 vArmor 覆盖。
...
**引用 Secret 不会让凭据从 Envoy 配置中消失。** vArmor 处理策略时读取 Secret，并将解析后的值写入 Envoy 监听器配置（`lds.yaml`）。该配置保存在 vArmor 生成的 Secret 中，并作为文件挂载到 sidecar；有权读取这个 Secret 或挂载文件的主体也能访问凭据。
...
3. 检查策略处理错误，等待配置文件更新并被代理加载。通过自有后端的非敏感指标确认新请求使用了更新值，不打印凭据。
...
轮换凭据时，还需安排已有连接的处理和应用信任库的重载，详见[生命周期与升级](lifecycle-and-upgrades.md)。
