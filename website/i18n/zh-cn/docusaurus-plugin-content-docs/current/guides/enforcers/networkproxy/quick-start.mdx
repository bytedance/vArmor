...
- 使用独立网络命名空间的 Linux Pod，集群准入策略允许注入 root 容器及 NET_ADMIN，见[安全与兼容性](security-and-compatibility.md)。
...
curl 对 403 也可能退出成功，应检查 HTTP 状态码。超时/拒绝连接不是本例预期的拒绝结果。初始配置不可用时参照[排障](troubleshooting.md)。

按[可观测性](observability.md)在节点或 sidecar 中查找审计，限定 namespace/Pod。
...
该规则变化不需要重建客户端 Pod。其他类型的变更请参阅[生命周期与升级](lifecycle-and-upgrades.md)。
