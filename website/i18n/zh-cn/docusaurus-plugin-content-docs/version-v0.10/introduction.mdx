...
- vArmor 的 NetworkProxy enforcer 进一步补充了 NetworkPolicy 的不足，提供了 HTTP/HTTPS 的 L7 访问控制（基于 TLS MITM）、基于 TLS SNI 的域名过滤、逐域名 HTTP 请求头注入、MITM 虚拟主机范围限制以及按条件选择的审计日志能力——这些是 NetworkPolicy 所不具备的。
:::
...
vArmor 还支持 NetworkProxy enforcer，通过 mutation webhook 向目标 Pod 注入 Envoy sidecar 代理和 init 容器。Init 容器通过 iptables 规则将出站流量重定向到 Envoy sidecar，后者根据 Manager 生成并通过统一 Secret 下发的策略执行 L4/L7 访问控制，包括 TLS MITM 终止、HTTPS 流量解密检查、HTTP 请求头注入、MITM 虚拟主机范围限制等能力。
...
- [执行器](guides/enforcers/index.md)：选择文件、系统调用或网络控制。
