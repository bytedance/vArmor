...
Curl can exit successfully for HTTP 403; check the HTTP code. A timeout or connection refusal is not the expected deny result for this example. If the initial configuration is not usable, inspect [Troubleshooting](troubleshooting.md).

Find the audit records at the node or sidecar location described in [Observability](observability.md), filtered to this namespace and Pod.
...
This rule change takes effect without recreating the client Pod. For other changes, see [Lifecycle and Upgrades](lifecycle-and-upgrades.md).
