...
func Test_versionGreaterThanOrEqual(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expectedErr:    fmt.Errorf("the current version (4) < the minimum version (%s)", minKernelVersionForAppArmorLSM),
		},
		{
			current:        "4.13.117.23.bsk.10-amd64",
			minimumVersion: minKernelVersionForAppArmorLSM,
			expectedResult: false,
			expectedErr:    fmt.Errorf("the current version (4.13.117) < the minimum version (%s)", minKernelVersionForAppArmorLSM),
		},
		{
			current:        "4.14.117.bsk.10-amd64",
			minimumVersion: minKernelVersionForAppArmorLSM,
			expectedResult: false,
			expectedErr:    fmt.Errorf("the current version (4.14.117) < the minimum version (%s)", minKernelVersionForAppArmorLSM),
		},
		{
			current:        "4.15",
			minimumVersion: minKernelVersionForAppArmorLSM,
		...
		},
		{
			...
			expectedResult: false,
			expectedErr:    fmt.Errorf("the current version (5.4.0) < the minimum version (%s)", minKernelVersionForBPFLSM),
		},
		{
			current:        "5.6.135-2-velinux1-amd64",
			minimumVersion: minKernelVersionForBPFLSM,
			expectedResult: false,
			expectedErr:    fmt.Errorf("the current version (5.6.135) < the minimum version (%s)", minKernelVersionForBPFLSM),
		},
		{
