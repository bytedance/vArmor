			...
			for _, mitm := range []bool{false, true} {
				for _, kind := range []string{"Pod", "Deployment", "StatefulSet", "DaemonSet"} {
					t.Run(fmt.Sprintf("%s/%s/uid_%d/mitm_%t", kind, tc.name, uid, mitm), func(t *testing.T) {
						...
						id := varmorpolicy.AuditPolicyIdentity{Kind: "VarmorPolicy", Name: "test", Namespace: "default"}
						// Apply admission JSON Patch to the actual object, then decode it back.
						// Repeat to ensure an annotated workload is not injected twice.
						for pass := 0; pass < 2; pass++ {
							patch, err := buildPatch(obj, "NetworkProxy", varmor.EnhanceProtectMode, target, config, "varmor-default-test", id, false, false)
							require.NoError(t, err)
							if patch == "" {
								assertProxySecurity(t, original, securityPodSpec(obj), uid, mitm)
								continue
							}
...
// Check decoded containers instead of relying on JSON field ordering.
func assertProxyPatchRootStart(t *testing.T, patch string) {
	t.Helper()
	var ops []struct {
		Path  string          `json:"path"`
		Value json.RawMessage `json:"value"`
	}
	require.NoError(t, json.Unmarshal([]byte("["+strings.TrimSuffix(patch, ",")+"]"), &ops))
	count := 0
	for _, op := range ops {
		if !strings.HasSuffix(op.Path, "/containers/-") && !strings.HasSuffix(op.Path, "/initContainers/-") {
			continue
		}
		var c corev1.Container
		require.NoError(t, json.Unmarshal(op.Value, &c))
